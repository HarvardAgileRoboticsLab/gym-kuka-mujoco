...
actor_options = {
    'learning_rate': lambda a: 1e-3,#(1-a)*1e-4 + a*1e-3,
    'gamma':1,
    'verbose':0,
    'n_steps':256,
    'ent_coef':0,
...
for i in range(100):
    ...
    envs = [gym.make('PegInsertionHole{}-v0'.format(i)) for _ in range(num_env)]
    ...
    try:
        ...
        actor_options['tensorboard_log'] = os.path.join(tensorboard_logdir,'PegInsertion-v0_cirriculum_learning')
        print('\t--learning')
        if i==0:
...
env = gym.make('PegInsertionHole99-v0')
